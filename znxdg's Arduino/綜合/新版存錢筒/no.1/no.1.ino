...
void setup() {
   pinMode(A0,INPUT);  //計數開關
   pinMode(A1,INPUT);  //清除開關
   pinMode(sp,OUTPUT); //蜂鳴器sp=A2
   pinMode(A3,INPUT);  //愛拚才會贏
   pinMode(A4,INPUT);  //青花瓷
   pinMode (7,OUTPUT); //馬達輸出1
   pinMode (8,OUTPUT); //馬達輸出2
   pinMode (A5,OUTPUT); //點心馬達輸出1
   pinMode (13,OUTPUT); //點心馬達輸出2
